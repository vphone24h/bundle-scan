...
BEGIN
  ...
  -- 7. Update debt_offsets (supplier_entity_id)
  UPDATE public.debt_offsets
  SET supplier_entity_id = _primary_id::text
  WHERE supplier_entity_id = ANY(SELECT unnest(_duplicate_ids)::text)
    AND tenant_id = _tenant_id;

  -- 8. Update debt_tag_assignments for supplier entities
