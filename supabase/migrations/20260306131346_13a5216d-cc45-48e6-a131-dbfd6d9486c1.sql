...
-- Fix get_customer_debt_summary: MAX(uuid) is not supported
CREATE OR REPLACE FUNCTION public.get_customer_debt_summary(_show_settled boolean DEFAULT false, _branch_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(entity_id uuid, entity_name text, entity_phone text, branch_id uuid, branch_name text, total_amount numeric, paid_amount numeric, remaining_amount numeric, first_debt_date timestamp with time zone, days_overdue integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Fix get_supplier_debt_summary: same MAX(uuid) issue
...
BEGIN
  ...
  WITH receipt_debts AS (
    SELECT
      ...
      (array_agg(ir.branch_id ORDER BY ir.import_date DESC))[1] AS r_branch_id
