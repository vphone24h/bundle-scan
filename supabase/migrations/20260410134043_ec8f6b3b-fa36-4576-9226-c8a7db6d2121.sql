...
BEGIN
  ...
  /*
    UPDATED FORMULA:
    totalSalesRevenue = SUM(total_amount) from export_receipts (ALL orders, not just items)
    totalReturnRevenue = SUM(refund_amount) from ALL export_returns
    netRevenue = totalSalesRevenue - totalReturnRevenue
    businessProfit = items profit (sale_price - import_price) - return profit loss
    netProfit = businessProfit + otherIncome - totalExpenses
  */

  -- 1a. Gross Revenue = SUM of receipt total_amount (includes services, repairs, etc.)
  ...
  ELSE
    -- When filtering by category, use items sum since receipt total includes all categories
    SELECT
  ...
  -- 1b. Business profit from items (always from items for accuracy)
  SELECT
