...
-- Global warranty lookup by IMEI (across all tenants)
CREATE OR REPLACE FUNCTION public.global_warranty_lookup_by_imei(_imei text, _ip_address text DEFAULT NULL)
RETURNS TABLE(
  id uuid, imei text, product_name text, sku text, warranty text, sale_price numeric,
  created_at timestamptz, branch_name text, export_date timestamptz,
  store_name text, tenant_id uuid,
...
-- Global warranty lookup by phone (across all tenants)
CREATE OR REPLACE FUNCTION public.global_warranty_lookup_by_phone(_phone text, _ip_address text DEFAULT NULL)
RETURNS TABLE(
  id uuid, imei text, product_name text, sku text, warranty text, sale_price numeric,
  created_at timestamptz, branch_name text, export_date timestamptz,
  store_name text, tenant_id uuid,
