...
CREATE OR REPLACE FUNCTION public.list_product_groups(
  ...
  p_printed_filter text DEFAULT NULL,
...
BEGIN
  ...
  -- Tenant from platform_users
  SELECT pu.tenant_id INTO v_tenant_id
  ...
  -- Role + branch from user_roles (scoped to tenant)
  SELECT ur.user_role::text, ur.branch_id
  INTO v_role, v_branch_id
  FROM public.user_roles ur
  WHERE ur.user_id = v_user_id AND ur.tenant_id = v_tenant_id
  LIMIT 1;

  v_can_view_all := COALESCE(v_role IN ('super_admin','manager'), false);
...
BEGIN
  ...
  SELECT pu.tenant_id INTO v_tenant_id
  FROM public.platform_users pu WHERE pu.user_id = v_user_id LIMIT 1;
  IF v_tenant_id IS NULL THEN RETURN '[]'::jsonb; END IF;

  SELECT ur.user_role::text, ur.branch_id INTO v_role, v_branch_id
  FROM public.user_roles ur
  WHERE ur.user_id = v_user_id AND ur.tenant_id = v_tenant_id LIMIT 1;
