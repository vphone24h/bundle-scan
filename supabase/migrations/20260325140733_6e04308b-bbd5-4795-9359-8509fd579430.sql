...
CREATE OR REPLACE FUNCTION public.get_inventory_summary(p_tenant_id uuid, p_branch_ids uuid[] DEFAULT NULL::uuid[])
  RETURNS TABLE(product_id uuid, product_name text, sku text, branch_id uuid, branch_name text, category_id uuid, category_name text, has_imei boolean, total_imported numeric, total_sold numeric, stock numeric, avg_import_price numeric, total_import_cost numeric, oldest_import_date timestamp with time zone, group_id uuid, variant_1 text, variant_2 text, variant_3 text, unit text)
  LANGUAGE sql
...
AS $function$
  WITH imei_deduped AS (
    SELECT DISTINCT ON (p.imei)
      ...
      p.group_id, p.variant_1, p.variant_2, p.variant_3,
  ...
  imei_summary AS (
    SELECT
      ...
      true as has_imei,
      COUNT(*)::numeric as total_imported,
      COUNT(*) FILTER (WHERE d.status = 'sold')::numeric as total_sold,
      COUNT(*) FILTER (WHERE d.status = 'in_stock')::numeric as stock,
      CASE
    ...
    GROUP BY d.name, d.sku, d.branch_id, b.name, d.category_id, c.name, d.group_id, d.variant_1, d.variant_2, d.variant_3
  ),
  non_imei_base AS (
    SELECT
      p.id, p.name, p.sku, p.branch_id, p.category_id,
  ...
  non_imei_summary AS (
    SELECT
      ...
      c.name as category_name,
      false as has_imei,
      SUM(nc.quantity)::numeric as total_imported,
      COALESCE(SUM(nc.quantity) FILTER (WHERE nc.status = 'sold'), 0)::numeric as total_sold,
      COALESCE(SUM(nc.quantity) FILTER (WHERE nc.status = 'in_stock'), 0)::numeric as stock,
      CASE
    ...
    GROUP BY nc.name, nc.sku, nc.branch_id, b.name, nc.category_id, c.name, nc.group_id, nc.variant_1, nc.variant_2, nc.variant_3
  )
