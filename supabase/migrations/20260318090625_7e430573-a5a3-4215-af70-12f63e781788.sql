...
-- Fix: import_receipts.status is enum receipt_status (only completed/cancelled)
-- Cast to text to avoid enum validation error with partial_return/full_return
...
BEGIN
  ...
  WITH per_supplier AS (
    ...
    WHERE ir.tenant_id = _tenant_id
      AND ir.status::text IN ('completed', 'partial_return', 'full_return')
