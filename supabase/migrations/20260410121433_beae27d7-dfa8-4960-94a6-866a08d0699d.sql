...
BEGIN
  ...
  -- COMBINED QUERY 1: Sales revenue + profit + products sold in ONE pass
  IF p_category_id IS NULL THEN
    SELECT
      COALESCE(SUM(er.total_amount), 0),
      COUNT(DISTINCT er.id),
      COALESCE(SUM(CASE WHEN eri.status = 'sold' THEN (eri.sale_price - COALESCE(p.import_price, 0)) * COALESCE(NULLIF(eri.quantity, 0), 1) ELSE 0 END), 0),
      COALESCE(SUM(CASE WHEN eri.status = 'sold' THEN 1 ELSE 0 END), 0)
    INTO _total_sales_revenue, _sales_count, _business_profit, _products_sold
    FROM public.export_receipts er
    LEFT JOIN public.export_receipt_items eri ON eri.receipt_id = er.id
  ...
  -- COMBINED QUERY 2: Returns revenue + profit loss in ONE pass
  IF p_category_id IS NULL THEN
    SELECT
      ...
      COUNT(*)
    INTO _total_return_revenue, _return_profit_loss, _products_returned
  ...
  -- COMBINED QUERY 3: Payments by source (only when no category filter)
  IF p_category_id IS NULL THEN
    SELECT
  ...
  -- COMBINED QUERY 4: Cash book expenses + income + expenses by category in ONE pass
  SELECT
