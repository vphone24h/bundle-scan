...
CREATE OR REPLACE FUNCTION public.merge_suppliers(_primary_id uuid, _duplicate_ids uuid[])
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- 1. products (supplier_id uuid)
  UPDATE public.products
  SET supplier_id = _primary_id
  WHERE supplier_id = ANY(_duplicate_ids)
    AND tenant_id = _tenant_id;

  -- 2. import_receipts (supplier_id uuid)
  UPDATE public.import_receipts
  SET supplier_id = _primary_id
  WHERE supplier_id = ANY(_duplicate_ids)
    AND tenant_id = _tenant_id;

  -- 3. import_returns (supplier_id uuid)
  UPDATE public.import_returns
  SET supplier_id = _primary_id
  WHERE supplier_id = ANY(_duplicate_ids);

  -- 4. product_imports (supplier_id uuid)
  UPDATE public.product_imports
  SET supplier_id = _primary_id
  WHERE supplier_id = ANY(_duplicate_ids);

  -- 5. stock_transfer_items (supplier_id uuid)
  UPDATE public.stock_transfer_items
  SET supplier_id = _primary_id
  WHERE supplier_id = ANY(_duplicate_ids);

  -- 6. debt_payments (entity_id uuid, tenant_id uuid)
  UPDATE public.debt_payments
  SET entity_id = _primary_id
  WHERE entity_type = 'supplier'
    AND entity_id = ANY(_duplicate_ids)
    AND tenant_id = _tenant_id;

  -- 7. debt_offsets (supplier_entity_id uuid, tenant_id uuid)
  UPDATE public.debt_offsets
  SET supplier_entity_id = _primary_id
  WHERE supplier_entity_id = ANY(_duplicate_ids)
    AND tenant_id = _tenant_id;

  -- 8. debt_tag_assignments (entity_id text, tenant_id text)
  UPDATE public.debt_tag_assignments
  SET entity_id = _primary_id::text
  WHERE entity_type = 'supplier'
    AND entity_id = ANY(ARRAY(SELECT unnest(_duplicate_ids)::text))
    AND tenant_id = _tenant_id::text;
  ...
  INSERT INTO public.audit_logs (
    ...
    jsonb_build_object('primary_id', _primary_id, 'merged_ids', _duplicate_ids)
  );
END;
$$;
