...
BEGIN
  ...
  IF _tag_id IS NOT NULL THEN
    -- Use JOIN for tag filter (faster than EXISTS subquery)
    SELECT
      COUNT(*)::integer,
      COUNT(*) FILTER (WHERE c.current_points > 0)::integer,
      COUNT(*) FILTER (WHERE c.membership_tier = 'vip')::integer,
      COUNT(*) FILTER (WHERE c.total_spent > 0)::integer
    INTO _total, _with_points, _vip, _purchased
    FROM public.customers c
    INNER JOIN public.customer_tag_assignments cta ON cta.customer_id = c.id AND cta.tag_id = _tag_id
