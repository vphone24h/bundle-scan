...
BEGIN
  ...
  /*
    LOCKED FORMULA - DO NOT CHANGE WITHOUT EXPLICIT BUSINESS APPROVAL
    totalSalesRevenue = SUM(sale_price * quantity) of export_receipt_items where status = 'sold'
    totalReturnRevenue = SUM(sale_price * quantity) of export_returns where fee_type = 'none'
    netRevenue = totalSalesRevenue - totalReturnRevenue
    productsSold = SUM(quantity) of sold items
    productsReturned = SUM(quantity) of returned items
  */
  ...
  SELECT
    ...
    COALESCE(SUM(ret.sale_price * COALESCE(NULLIF(ret.quantity, 0), 1)), 0),
    COALESCE(SUM(COALESCE(NULLIF(ret.quantity, 0), 1)), 0)
  INTO _return_profit_loss, _total_return_revenue, _products_returned
  ...
  FROM (
    SELECT
      ...
      SUM(eri.sale_price * COALESCE(NULLIF(eri.quantity, 0), 1))::numeric AS revenue,
      SUM((eri.sale_price - COALESCE(p.import_price, 0)) * COALESCE(NULLIF(eri.quantity, 0), 1))::numeric AS profit,
      COUNT(*)::integer AS count
    FROM public.export_receipt_items eri
    JOIN public.export_receipts er ON er.id = eri.receipt_id
    LEFT JOIN public.products p ON p.id = eri.product_id
    LEFT JOIN public.categories cat ON cat.id = eri.category_id
    WHERE er.tenant_id = p_tenant_id
      AND er.status != 'cancelled'
      AND eri.status = 'sold'
      AND er.export_date >= _start_ts
...
COMMENT ON FUNCTION public.get_report_stats_aggregated(uuid, text, text, uuid, uuid, text)
IS 'LOCKED BUSINESS FORMULA: totalSalesRevenue = tong gia ban cua cac dong sold; totalReturnRevenue = tong gia tri tra hang; netRevenue = sales - returns; productsSold/productsReturned = tong so luong. Chi duoc thay doi khi co phe duyet nghiep vu ro rang.';