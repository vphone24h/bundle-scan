...
BEGIN
  ...
  IF p_category_id IS NULL THEN
    -- Khong loc danh muc: tinh tu export_receipts
    SELECT 
  ...
  ELSE
    -- Co loc danh muc: tinh tu export_receipt_items theo category
    SELECT 
  ...
  -- 2. Doanh thu tra hang (cung loc theo category neu co)
  IF p_category_id IS NULL THEN
    SELECT 
  ...
  -- 3. Payment sources (loc theo category neu co)
  IF p_category_id IS NULL THEN
    SELECT 
    ...
    WHERE er.tenant_id = p_tenant_id
      ...
      AND (p_branch_id IS NULL OR er.branch_id = p_branch_id);
  ELSE
    -- Khi loc category, payment sources khong ap dung duoc chinh xac, de 0
    _pay_cash := 0;
    _pay_bank := 0;
    _pay_ewallet := 0;
    _pay_debt := 0;
  ...
  -- 4. So quy (khong anh huong boi category)
  SELECT 
