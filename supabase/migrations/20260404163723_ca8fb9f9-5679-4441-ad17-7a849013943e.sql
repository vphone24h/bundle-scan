...
DECLARE
  ...
  _result json;
BEGIN
  SELECT tenant_id, branch_id INTO _tenant_id, _user_branch_id
  ...
  -- Use user's branch if no specific branch filter and user has a branch
  IF _branch_id IS NULL AND _user_branch_id IS NOT NULL THEN
    _branch_id := _user_branch_id;
  END IF;
