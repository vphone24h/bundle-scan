...
-- Add indexes for faster search on import_receipts and export_receipts
CREATE INDEX IF NOT EXISTS idx_import_receipts_code_trgm ON import_receipts USING gin (code gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_export_receipts_code_trgm ON export_receipts USING gin (code gin_trgm_ops);
...
-- RPC for fast export receipt search (single query with JOIN instead of 2 sequential queries)
...
BEGIN
  RETURN QUERY
  SELECT
    er.id, er.code, er.export_date, er.status,
...
-- RPC for fast import receipt search
CREATE OR REPLACE FUNCTION search_import_receipts(
  _search text DEFAULT NULL,
...
BEGIN
  ...
  SELECT
    ...
    ir.total_amount, ir.paid_amount, ir.remaining_amount,
