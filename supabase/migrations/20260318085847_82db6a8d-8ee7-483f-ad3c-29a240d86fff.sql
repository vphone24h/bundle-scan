...
-- Fix: Include partial_return and full_return receipts that still have remaining debt
-- Previously only 'completed' status was included, causing debt to disappear after returns
...
-- Also fix supplier debt summary for consistency
...
BEGIN
  ...
  WITH per_supplier AS (
    ...
    WHERE ir.tenant_id = _tenant_id
      AND ir.status IN ('completed', 'partial_return', 'full_return')
