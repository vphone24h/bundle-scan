...
AS $$
  ...
  -- Products that belong to a product_group
  grouped_products AS (
    SELECT
    ...
    WHERE p.tenant_id = tf.tid
      AND p.status IN ('in_stock', 'template')
      AND (
  ...
  -- Standalone products that have variant fields but no group_id -> collapse by base name
  variant_no_group AS (
    SELECT
      -- Strip variant values from the name to get the base name
      TRIM(
        REPLACE(
          REPLACE(
  ...
  -- Standalone products without any variant info
  ...
  combined AS (
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM grouped_products gp WHERE gp.product_name = vng.product_name
    )
    UNION ALL
    SELECT s.*
    ...
    AND NOT EXISTS (
      SELECT 1 FROM variant_no_group vng WHERE vng.product_name = s.product_name
    )
  )
  SELECT *
  FROM combined
