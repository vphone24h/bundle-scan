...
CREATE OR REPLACE FUNCTION public.lookup_warranty_by_imei(_imei text, _tenant_id uuid)
 RETURNS TABLE(id uuid, imei text, product_name text, sku text, warranty text, sale_price numeric, created_at timestamp with time zone, branch_name text, export_date date, staff_user_id uuid, staff_name text, branch_id uuid, customer_name text, customer_id uuid, customer_phone text)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  -- First find the customer who owns this IMEI, then return ALL their purchases
