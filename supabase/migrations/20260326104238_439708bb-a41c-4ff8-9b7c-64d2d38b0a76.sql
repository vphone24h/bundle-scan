...
BEGIN
  ...
  LOOP
    ...
    -- Inventory: sum of (import_price * quantity) from product_imports up to date d
    -- minus sold items from export_receipt_items up to date d
    SELECT COALESCE(SUM(
    ...
    FROM debt_payments 
    WHERE tenant_id = _tenant_id AND entity_type = 'customer' AND created_at::DATE <= d;
    ...
    FROM debt_payments 
    WHERE tenant_id = _tenant_id AND entity_type = 'supplier' AND created_at::DATE <= d;
