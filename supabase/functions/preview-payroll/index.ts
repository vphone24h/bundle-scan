...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const target_user_id = (body.user_id as string) || user.id;
    if (!tenant_id) throw new Error("Missing tenant_id");

    // Build virtual period: current month from day 1 to today
    const now = new Date();
    const periodStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const periodEnd = new Date(now.getFullYear(), now.getMonth() + 1, 0); // last day of month
    const ymd = (d: Date) => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
    const period: any = {
      start_date: ymd(periodStart),
      end_date: ymd(periodEnd),
      status: "draft",
    };
    const period_id = "preview";
    ...
    // Filter to single target user
    const filteredScoped = scopedUserIds.filter((id) => id === target_user_id);
    if (!filteredScoped.length) {
      return new Response(JSON.stringify({ success: false, error: "User not in tenant scope" }), {
    ...
    const employees = filteredScoped.map((userId) => {
    ...
    for (const employee of employees) {
      ...
      const approvedExtraHoursMinutes = new Map<string, number>();
      for (const r of userOTRequests.filter((r: any) => r.request_type === "extra_hours")) {
        approvedExtraHoursMinutes.set(r.request_date, (approvedExtraHoursMinutes.get(r.request_date) || 0) + (r.overtime_minutes || 0));
      }
          ...
          if ((c.target_type === "product" || c.target_type === "service") && c.target_id) {
            ...
            if (sold && sold.revenue > 0) {
              ...
              commissionDetails.push({
                ...
                target_type: c.target_type,
                qty: 0, revenue: 0, rate: c.value, calc_type: c.calc_type,
                amount: 0, products: [], only_self_sold: true,
              });
            }
            ...
            if (_userRevenue > 0) {
              ...
              commissionDetails.push({
                ...
                target_type: "revenue",
                revenue: 0, rate: c.value, calc_type: c.calc_type,
                amount: 0, products: [], only_self_sold: true,
              });
            }
    ...
    // DRY-RUN: do not persist, return single record + extras
