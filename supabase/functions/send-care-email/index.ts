...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { customerIds, subject, htmlContent } = body

    if (!customerIds || !Array.isArray(customerIds) || customerIds.length === 0) {
      return new Response(JSON.stringify({ error: 'Thiếu danh sách khách hàng' }), {
        status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }

    if (customerIds.length > 200) {
      return new Response(JSON.stringify({ error: 'Tối đa 200 khách hàng mỗi lần gửi' }), {
        status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
    ...
    if (customersWithEmail.length === 0) {
      return new Response(JSON.stringify({ error: 'Không có khách hàng nào có email' }), {
        status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
...
<body style="margin:0;padding:0;background:#f0f4ff;font-family:Arial,Helvetica,sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f4ff;padding:32px 16px">
    ...
    return new Response(JSON.stringify({ 
      sent, 
      failed, 
      total: customersWithEmail.length,
      skipped: customerIds.length - customersWithEmail.length,
      failedEmails,
    }), {
    ...
    console.error('send-care-email error:', error)
