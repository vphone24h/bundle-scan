...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: allowed } = await rlClient.rpc('check_rate_limit', { _function_name: 'restore-product-metadata', _ip_address: clientIP, _max_requests: 5, _window_minutes: 60 })
    if (allowed === false) {
