...
export default function RepairListPage() {
  ...
  const handlePrintRepairReceipt = async (order: RepairOrder) => {
    ...
    `;
    // Use iframe to avoid popup blocker on mobile
                      ...
                      <div key={item.id} className="flex items-start justify-between p-2 border rounded text-sm">
                        ...
                        {selectedOrder.status !== 'returned' && selectedOrder.status !== 'cancelled' && (
                          <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0" onClick={async () => {
                            ...
                            const newTotal = totalService + totalParts;
                            await updateOrder.mutateAsync({ id: selectedOrder.id, total_service_price: totalService, total_parts_price: totalParts, total_parts_cost: totalPartsCost, total_amount: newTotal } as any);
                          }}>
