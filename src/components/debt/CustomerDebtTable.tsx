...
import { useCustomerDebts, DebtSummary } from '@/hooks/useDebt';
...
import { Eye, Wallet, Plus, Printer, MoreHorizontal, UserPlus, Hash, Phone, MessageSquare } from 'lucide-react';
...
export function CustomerDebtTable({ showSettled, branchFilter, tagFilter, quickFilter = 'all', overdueDays = 15 }: CustomerDebtTableProps) {
      ...
      <DropdownMenuContent align="end" className="bg-popover">
        ...
        {debt.entity_phone && (
          <>
            ...
            <DropdownMenuItem onClick={() => window.open(`sms:${debt.entity_phone}?body=Xin chào ${debt.entity_name}, nhắc nhở thanh toán công nợ ${formatNumber(debt.remaining_amount)}đ. Cảm ơn!`, '_self')}>
              <MessageSquare className="mr-2 h-4 w-4" /> SMS nhắc nợ
            </DropdownMenuItem>
          </>
        )}
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="text-center py-12 text-muted-foreground">
          {tagFilter ? 'Không có công nợ nào với hashtag này' : 'Không có công nợ khách hàng'}
        </div>
  ...
  return (
    <>
      ...
      {selectedDebt && (
        <>
          <DebtDetailDialog open={showDetail} onOpenChange={setShowDetail} entityType="customer"
          ...
          <DebtAdditionDialog open={showAddition} onOpenChange={setShowAddition} entityType="customer"
            entityId={selectedDebt.entity_id} entityName={selectedDebt.entity_name}
            remainingAmount={selectedDebt.remaining_amount} branchId={selectedDebt.branch_id} />
