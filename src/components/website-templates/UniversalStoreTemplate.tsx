          ...
          <div>
            {config.homeSections.map((sectionId) => {
              switch (sectionId) {
                ...
                case 'branches':
                  // Merged into storeInfo section below
                  return null;
                case 'storeInfo':
                  if (!config.features.storeInfo || settings?.show_store_info === false) return null;
                  const hasContactInfo = settings?.store_phone || settings?.store_email || zaloUrl || facebookUrl || tiktokUrl;
                      ...
                      <section className="py-12 bg-[#f5f5f7]">
                        <div className="max-w-[1200px] mx-auto px-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            {hasContactInfo && (
                              <div>
                                <h2 className="text-xl font-bold tracking-tight mb-4">Liên hệ</h2>
                                <div className="bg-white rounded-2xl p-5 space-y-1">
                                  {settings?.store_phone && (
                                    <a href={`tel:${settings.store_phone}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#f5f5f7] transition-colors" style={{ color: accentColor }}>
                                      ...
                                      <div>
                                        <p className="text-[11px] text-[#86868b]">Số điện thoại</p>
                                  ...
                                  {zaloUrl && (
                                    <a href={zaloUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#f5f5f7] transition-colors" style={{ color: accentColor }}>
                                      ...
                                      <div>
                                        <p className="text-[11px] text-[#86868b]">Zalo</p>
                                  ...
                                  {facebookUrl && (
                                    <a href={facebookUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#f5f5f7] transition-colors" style={{ color: accentColor }}>
                                      ...
                                      <div>
                                        <p className="text-[11px] text-[#86868b]">Facebook</p>
                                  ...
                                  {tiktokUrl && (
                                    <a href={tiktokUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#f5f5f7] transition-colors" style={{ color: accentColor }}>
                                      <ExternalLink className="h-5 w-5 shrink-0" />
                                      <div>
                                        <p className="text-[11px] text-[#86868b]">TikTok</p>
                            ...
                            {branches.length > 0 && (
                              <div>
                                <h2 className="text-xl font-bold tracking-tight mb-4">Chi nhánh</h2>
                                <div className={`grid grid-cols-1 ${branches.length > 1 ? 'sm:grid-cols-2' : ''} gap-3`}>
                                  {branches.map(branch => (
                                    <div key={branch.id} className="bg-white rounded-2xl p-4 space-y-2">
                    ...
                    switch (section.id) {
                      ...
                      case 'categoryFilter': {
                        ...
                        if (newsCatMode === 'vertical') {
                          // Vertical: clean folder-list look (like vphone.vn). Children hidden until parent clicked.
                          ...
                          const renderNewsCatNode = (cat: any, depth: number = 0): JSX.Element => {
                            ...
                            return (
                              <div key={cat.id}>
                                ...
                                >
                                  <Folder className="h-5 w-5 shrink-0" style={{ color: isSelected ? accentColor : '#86868b' }} />
                                  <span
                                    className="text-[15px] font-medium flex-1"
                                    style={{ color: isSelected ? accentColor : '#1d1d1f' }}
                                  >
                                    {cat.name}
                                  </span>
                                  {hasChildren && (
                                    <ChevronDown
                                      className="h-4 w-4 text-[#86868b] transition-transform"
                                      style={{ transform: isExpanded ? 'rotate(0deg)' : 'rotate(-90deg)' }}
                                    />
                                  )}
                                </button>
                                {hasChildren && isExpanded && (
                                  <div className="flex flex-col border-l border-black/10 ml-5">
                          ...
                          return (
                            <div key="categoryFilter" className="mb-6">
                              <h3 className="text-sm font-bold tracking-wider text-[#1d1d1f] mb-3 uppercase">DANH MỤC</h3>
