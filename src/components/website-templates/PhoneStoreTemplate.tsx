...
interface PhoneStoreTemplateProps {
...
export default function PhoneStoreTemplate({
  ...
  const primaryColor = settings?.primary_color || '#1d1d1f';
  ...
  const handleWarrantyLogout = () => {
    if (warrantyStorageKey) localStorage.removeItem(warrantyStorageKey);
    setSearchValue(''); setSubmittedValue(''); setPageView('home');
  };

  const handleSearch = () => {
    if (searchValue.trim()) { setSubmittedValue(searchValue.trim()); if (pageView === 'home') setPageView('warranty'); }
  };
  ...
  return (
    <div className="min-h-screen bg-white text-[#1d1d1f]" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", "Segoe UI", Roboto, sans-serif' }}>
      {/* === APPLE-STYLE HEADER === */}
          ...
          <div className="flex items-center justify-between h-12">
            ...
            <nav className="flex items-center gap-1">
              {[
                ...
                { id: 'products' as PageView, label: 'Sản phẩm' },
                { id: 'news' as PageView, label: 'Tin tức' },
                { id: 'warranty' as PageView, label: 'Bảo hành' },
              ].map(item => (
                <button
                  key={item.id}
        ...
        {pageView === 'home' && (
          <div>
            ...
            {settings?.show_banner && settings?.banner_image_url ? (
              ...
              <section className="relative overflow-hidden bg-gradient-to-b from-[#000000] to-[#1d1d1f] text-white">
                <div className="max-w-[1200px] mx-auto px-6 py-16 sm:py-24 text-center">
                  ...
                  <p className="text-lg sm:text-xl text-white/70 mb-8 max-w-lg mx-auto">
                    Siêu phẩm công nghệ. Giá tốt nhất thị trường.
                  </p>
                  <div className="flex items-center justify-center gap-3 flex-wrap">
                    <Button
                      onClick={() => navigateTo('products')}
                      className="bg-[#0071e3] hover:bg-[#0077ed] text-white rounded-full px-8 h-11 text-sm font-medium"
                    >
                      Mua ngay
                    </Button>
                    <Button
            ...
            <section className="bg-white border-b border-black/5">
              <div className="max-w-[1200px] mx-auto px-4 py-6">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    { icon: <Shield className="h-5 w-5" />, title: 'Bảo hành 12 tháng', desc: 'Chính hãng Apple' },
                    { icon: <Award className="h-5 w-5" />, title: 'Hàng chính hãng', desc: '100% nguyên seal' },
                    { icon: <Truck className="h-5 w-5" />, title: 'Giao nhanh 2h', desc: 'Nội thành HCM/HN' },
                    { icon: <CreditCard className="h-5 w-5" />, title: 'Trả góp 0%', desc: 'Duyệt nhanh 15 phút' },
                  ].map((item, i) => (
            ...
            {/* FEATURED PRODUCTS - Apple grid style */}
            {featuredProducts.length > 0 && (
              <section className="py-12 bg-white">
                <div className="max-w-[1200px] mx-auto px-4">
                  <div className="text-center mb-8">
                    <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Sản phẩm nổi bật</h2>
                    <p className="text-sm text-[#86868b] mt-1">Được khách hàng yêu thích nhất</p>
                  </div>
                  ...
                  <div className="text-center mt-8">
                    <Button
                      variant="outline"
                      onClick={() => navigateTo('products')}
                      className="rounded-full px-8 h-10 text-sm font-medium border-[#0071e3] text-[#0071e3] hover:bg-[#0071e3]/5"
                    >
                      Xem tất cả sản phẩm →
                    </Button>
                  </div>
            ...
            {featuredProducts.length === 0 && allProducts.length > 0 && (
              <section className="py-12 bg-white">
                <div className="max-w-[1200px] mx-auto px-4">
                  <div className="text-center mb-8">
                    <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Sản phẩm</h2>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
                  ...
                  {allProducts.length > 8 && (
                    <div className="text-center mt-8">
                      <Button variant="outline" onClick={() => navigateTo('products')} className="rounded-full px-8 h-10 text-sm font-medium border-[#0071e3] text-[#0071e3] hover:bg-[#0071e3]/5">
            ...
            {featuredArticles.length > 0 && (
              <section className="py-12 bg-[#f5f5f7]">
                <div className="max-w-[1200px] mx-auto px-4">
                  <div className="text-center mb-8">
                    <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Tin tức mới</h2>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {featuredArticles.slice(0, 3).map(a => (
                      <button key={a.id} onClick={() => openArticle(a)} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all text-left group">
                        {a.thumbnail_url ? (
                        ...
                        <div className="p-5">
                          ...
                          <p className="text-[11px] text-[#0071e3] font-medium mt-3">Đọc thêm →</p>
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            {(settings?.show_warranty_lookup !== false) && (
              <section className="py-12 bg-white">
                <div className="max-w-lg mx-auto px-4 text-center">
                  <Shield className="h-8 w-8 mx-auto text-[#0071e3] mb-3" />
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      className="flex-1 h-12 text-base rounded-xl border-black/10 focus-visible:ring-[#0071e3]"
                      inputMode="tel"
                    />
                    <Button onClick={handleSearch} disabled={!searchValue.trim() || isSearching} className="h-12 px-6 rounded-xl bg-[#0071e3] hover:bg-[#0077ed]">
            ...
            {tenantId && (
              <section className="py-12 bg-white">
                <div className="max-w-[1200px] mx-auto px-4">
                  <StoreReviewsSection tenantId={tenantId} primaryColor="#0071e3" />
                </div>
              </section>
            )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {branches.map(branch => (
                      <div key={branch.id} className="bg-white rounded-2xl p-5 space-y-3">
                        ...
                        {branch.phone && (
                          <a href={`tel:${branch.phone}`} className="flex items-center gap-2 text-[#0071e3]">
                    ...
                    {branches.length === 0 && (
                      <div className="bg-white rounded-2xl p-5 space-y-3">
                        ...
                        {settings?.store_phone && (
                          <a href={`tel:${settings.store_phone}`} className="flex items-center gap-2 text-[#0071e3]">
                            <Phone className="h-4 w-4" />
                            <p className="text-sm font-medium">{settings.store_phone}</p>
                          </a>
                        )}
                        {settings?.store_email && (
                          <a href={`mailto:${settings.store_email}`} className="flex items-center gap-2 text-[#0071e3]">
              ...
              <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide">
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs font-medium whitespace-nowrap transition-all ${
                    !selectedCategoryId ? 'bg-[#1d1d1f] text-white' : 'bg-[#f5f5f7] text-[#1d1d1f] hover:bg-black/10'
                  }`}
                >
                  Tất cả
                </button>
                {productsData.categories.map(cat => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-xs font-medium whitespace-nowrap transition-all ${
                      selectedCategoryId === cat.id ? 'bg-[#1d1d1f] text-white' : 'bg-[#f5f5f7] text-[#1d1d1f] hover:bg-black/10'
                    }`}
                  >
                    {cat.name}
                  </button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
              ...
              {filteredProducts.length === 0 && (
                <div className="col-span-full text-center py-16">
                  ...
                  <p className="text-sm text-[#86868b]">Chưa có sản phẩm nào</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="max-w-[1200px] mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {articlesData?.articles?.map(a => (
                <button key={a.id} onClick={() => openArticle(a)} className="bg-white rounded-2xl overflow-hidden border border-black/5 hover:shadow-lg transition-all text-left group">
                  {a.thumbnail_url ? (
                    <img src={a.thumbnail_url} alt={a.title} className="w-full h-48 object-cover group-hover:scale-[1.02] transition-transform" />
                  ) : (
                    <div className="w-full h-48 bg-[#f5f5f7] flex items-center justify-center"><Newspaper className="h-10 w-10 text-[#86868b]" /></div>
        ...
        {pageView === 'article-detail' && selectedArticle && (
          <div className="max-w-3xl mx-auto px-4 py-8">
            <div className="flex items-center justify-between mb-6">
              <button onClick={() => navigateTo('news')} className="flex items-center gap-2 text-sm text-[#0071e3] hover:underline">
            ...
            <article>
              ...
              {selectedArticle.content && (
                <div
                  className="prose prose-sm max-w-none [&_img]:max-w-full [&_img]:h-auto [&_img]:rounded-2xl [&_a]:text-[#0071e3] [&_a]:underline"
        ...
        {pageView === 'warranty' && (
          <div className="max-w-lg mx-auto px-4 py-8 space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold tracking-tight">Tra cứu bảo hành</h2>
            </div>
            ...
            <div className="bg-[#f5f5f7] rounded-2xl p-5 space-y-4">
              <div className="flex gap-2">
                <Input
                  ...
                  onChange={e => setSearchValue(e.target.value)}
                  onKeyPress={handleKeyPress}
                  className="flex-1 h-12 text-base rounded-xl border-black/10 bg-white focus-visible:ring-[#0071e3]"
                  inputMode="tel"
                />
                <Button onClick={handleSearch} disabled={!searchValue.trim() || isSearching} className="h-12 px-6 rounded-xl bg-[#0071e3] hover:bg-[#0077ed]">
                  {isSearching ? <Loader2 className="h-5 w-5 animate-spin" /> : <Search className="h-5 w-5" />}
              ...
              {warrantyHotline && (
                <a href={`tel:${warrantyHotline}`} className="flex items-center gap-3 p-3 rounded-xl bg-white">
                  <div className="p-2 rounded-full bg-[#0071e3]/10"><Headphones className="h-4 w-4 text-[#0071e3]" /></div>
                  <div className="flex-1">
                    <p className="text-[10px] text-[#86868b]">Hotline bảo hành</p>
                    <p className="text-sm font-semibold text-[#0071e3]">{warrantyHotline}</p>
                  </div>
                </a>
              )}
              {supportGroupUrl && (
      ...
      {(zaloUrl || warrantyHotline) && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-xl border-t border-black/5 py-2 px-4 sm:hidden" style={{ paddingBottom: 'max(8px, env(safe-area-inset-bottom))' }}>
          <div className="flex items-center gap-2">
            {zaloUrl && (
              <a href={zaloUrl} target="_blank" rel="noopener noreferrer" className="flex-1 bg-[#0071e3] text-white rounded-xl py-2.5 text-center text-sm font-medium">
                Chat Zalo
              </a>
            )}
            {warrantyHotline && (
...
function ProductCard({ product, onClick }: { product: LandingProduct; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="bg-[#f5f5f7] rounded-2xl overflow-hidden text-left group transition-all hover:shadow-lg"
    >
