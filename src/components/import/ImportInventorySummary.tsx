...
import { Package, DollarSign, Archive, ShoppingCart, EyeOff } from 'lucide-react';
...
export function ImportInventorySummary({ isFiltered = false, filteredProducts }: ImportInventorySummaryProps) {
  ...
  const inventoryInStockQty = useMemo(() => {
    if (!inventory) return 0;
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      // Use inventory-synced values for in-stock
      inStockValue: inventoryInStockValue || Number(serverStats.inStockValue || 0),
      inStockQuantity: inventoryInStockQty || Number(serverStats.inStockQuantity || 0),
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="p-1.5 sm:p-2 rounded-lg bg-primary/20 flex-shrink-0">
                {valueHidden ? <EyeOff className="h-4 w-4 sm:h-5 sm:w-5 text-muted-foreground" /> : <DollarSign className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[10px] sm:text-xs text-muted-foreground truncate">{isFiltered ? t('importSummary.totalImportValueFiltered') : t('importSummary.totalImportValue')}</p>
                <p className={`text-sm sm:text-lg font-bold ${valueHidden ? 'text-muted-foreground' : 'text-primary'}`} title={formatCurrency(stats.totalImportValue)}>{renderValue(stats.totalImportValue)}</p>
                <p className="text-[10px] sm:text-xs text-muted-foreground">{renderQty(stats.totalQuantity)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="p-1.5 sm:p-2 rounded-lg bg-green-500/20 flex-shrink-0">
                {valueHidden ? <EyeOff className="h-4 w-4 sm:h-5 sm:w-5 text-muted-foreground" /> : <Archive className="h-4 w-4 sm:h-5 sm:w-5 text-green-600 dark:text-green-400" />}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[10px] sm:text-xs text-muted-foreground truncate">{t('importSummary.inStockValue')}</p>
                <p className={`text-sm sm:text-lg font-bold ${valueHidden ? 'text-muted-foreground' : 'text-green-600 dark:text-green-400'}`} title={formatCurrency(stats.inStockValue)}>{renderValue(stats.inStockValue)}</p>
                <p className="text-[10px] sm:text-xs text-muted-foreground">{renderQty(stats.inStockQuantity)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="p-1.5 sm:p-2 rounded-lg bg-blue-500/20 flex-shrink-0">
                {valueHidden ? <EyeOff className="h-4 w-4 sm:h-5 sm:w-5 text-muted-foreground" /> : <ShoppingCart className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600 dark:text-blue-400" />}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[10px] sm:text-xs text-muted-foreground truncate">{t('importSummary.soldValue')}</p>
                <p className={`text-sm sm:text-lg font-bold ${valueHidden ? 'text-muted-foreground' : 'text-blue-600 dark:text-blue-400'}`} title={formatCurrency(stats.soldValue)}>{renderValue(stats.soldValue)}</p>
                <p className="text-[10px] sm:text-xs text-muted-foreground">{renderQty(stats.soldQuantity)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="p-1.5 sm:p-2 rounded-lg bg-orange-500/20 flex-shrink-0">
                {valueHidden ? <EyeOff className="h-4 w-4 sm:h-5 sm:w-5 text-muted-foreground" /> : <Package className="h-4 w-4 sm:h-5 sm:w-5 text-orange-600 dark:text-orange-400" />}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[10px] sm:text-xs text-muted-foreground truncate">{t('importSummary.returnedValue')}</p>
                <p className={`text-sm sm:text-lg font-bold ${valueHidden ? 'text-muted-foreground' : 'text-orange-600 dark:text-orange-400'}`} title={formatCurrency(stats.returnedValue)}>{renderValue(stats.returnedValue)}</p>
                <p className="text-[10px] sm:text-xs text-muted-foreground">{renderQty(stats.returnedQuantity)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
